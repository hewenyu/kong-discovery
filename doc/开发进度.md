...
随着系统使用范围的扩大，我们收到了用户需求，希望能够支持按命名空间隔离服务，以便更好地管理不同环境、项目或团队的服务。为此，我们计划增加命名空间功能，使系统能够更加灵活地支持多租户场景。

### 任务5.5.1：核心命名空间功能（计划中）
- [ ] 更新数据模型，添加命名空间字段
  - 在Service结构体中添加Namespace字段
  - 在DNS记录中添加Namespace字段
  - 设计Namespace实体结构
- [ ] 实现命名空间管理接口（增删改查）
  - 创建命名空间API
  - 删除命名空间API
...
### 任务5.5.2：DNS命名空间解析（计划中）
- [ ] 更新DNS记录生成逻辑，支持命名空间
  - 生成带命名空间前缀的DNS记录
  - 兼容现有的不带命名空间的DNS查询
- [ ] 实现命名空间前缀的DNS解析
  - 支持"service.namespace.domain"格式
  - 支持"service.domain"格式（使用默认命名空间）
- [ ] 优化命名空间DNS查询性能
  - 针对命名空间查询优化缓存策略
  - 提高命名空间查询的响应速度
- [ ] **测试**：验证命名空间DNS解析功能

### 任务5.5.3：SDK命名空间支持（计划中）
- [ ] 更新Go SDK支持命名空间
  - 在Config结构体中添加Namespace字段
  - 默认使用"default"命名空间
  - 注册服务时传递命名空间信息
- [ ] 更新SDK示例代码
  - 添加使用命名空间的示例
...
### 命名空间存储设计
命名空间信息将存储在etcd中，使用以下键值结构：
```
/namespaces/{namespace} -> JSON格式的命名空间信息
/services/{namespace}/{serviceId} -> JSON格式的服务信息
```

### DNS解析设计
DNS解析将支持以下格式：
1. `service.namespace.service.local` - 指定命名空间下的服务
2. `service.service.local` - 默认命名空间下的服务

### API接口设计
1. 命名空间管理API (9090端口)：
   - GET /api/v1/namespaces - 获取命名空间列表
   - GET /api/v1/namespaces/{namespace} - 获取命名空间详情
   - POST /api/v1/namespaces - 创建命名空间
   - DELETE /api/v1/namespaces/{namespace} - 删除命名空间

2. 服务注册API (8080端口) 更新：
   - 注册请求体添加namespace字段
   - 服务ID生成逻辑更新，确保跨命名空间唯一

## 下一步计划
接下来我们将首先实现核心命名空间功能，更新数据模型和API接口，然后逐步完善DNS解析、SDK和前端支持。命名空间功能的引入将显著提升系统的多租户支持能力，为大规模微服务部署提供更好的隔离和管理。 